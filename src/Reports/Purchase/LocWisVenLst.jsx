...
import CityLuv from '../../Luvs/CityLuv';
...
const LocWisVenLst = () => {
    ...
    const handleCityDtls = async (data) => {
        setCityCd(data.PUCM_CITY_CD);
        setZone(data.PUCM_ZONE);
    }

    const getReports = async () => {
        if(cityCd === null || !cityCd){
            toast.info('Select City.');
            return;
        }
        setLoader(true);
        let path = '/reports/Purchase/LocwiseVendorListRep';
        const params = {
        ...
        const printStatus = await onPrintRepJAS(outTypeVal, "Locwise Vendor List Report", path, params);
        printStatus && setLoader(false);
        setLoader(false);
    }

    const validateCity = async() =>{
        let where = `AND PUCM_CITY_CD like` + "'%" + cityCd.toUpperCase() + "%' ";
        const result = await axios.post('/api/genericLuv/getCityList', { where, orgId });   
        if (result.data.data.length > 1) {
            setCityCd('');
            toast.info('Invalid City');
            return;
        }
    }
        ...
        <>
            ...
            <div className='parentDivEpForm ' style={blurBackgroundStyle}>
                ...
                <div className='formContDiv' style={{ width: helpScreen ? '75%' : '80%', marginLeft: helpScreen ? '2%' : 'auto' }}>
                    <FormHeading adrmRightId='319' headingText='Location Wise Vendor Listing' style={{ zIndex: '0' }} />
                        ...
                        <div className="row d-flex mt-3 w-6" style={{ height: '4vh', marginLeft: 'auto', marginRight: 'auto' }}>  
                            <div className="col-md-2 w-6 p-0 m-0" style={{ height: '4vh' }}>
                                <div className='inputTagHeight w-14'>
                                    <InputTagWithLabel text='City Code' value={cityCd} onBlur={() => { validateCity() }} funCall={() => { setShowLuv(true) }}
                                        onChange={(e) => setCityCd(e.target.value)} searchWidth='65%' readOnly={'false'} display='true' />
                                </div>
                            </div>
                        </div>
