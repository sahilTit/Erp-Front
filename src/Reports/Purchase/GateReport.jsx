...
const GateReport = () => {
    ...
    const [vendCode, setVendorCode] = useState()
    ...
    const [venApmId, setVenApmId] = useState('');
    ...
    const getVendorList = async () => {
        try {
            const res = await axios.post('/api/reports/purchase/gateEntry/getVendorList', { page });
            if (res.data.vendorList) {
                setVendorList(res.data.vendorList);
                const len = res.data.totCount;
    ...
    const filterVendorData = async () => {
        ...
        if (searchVendorName !== undefined && searchVendorName !== null && searchVendorName !== '') {
            where = ` and P.APM_NAME LIKE ` + "'%" + searchVendorName.toUpperCase() + "%' ";
        }
        if (searchVendorCode !== undefined && searchVendorCode !== null && searchVendorCode !== '') {
            where = where + ` and P.APM_CD LIKE ` + "'%" + searchVendorCode.toUpperCase() + "%' ";
        }
        if (searchVendorId !== undefined && searchVendorId !== null && searchVendorId !== '') {
            where = where + ` and P.APM_ID LIKE ` + "'%" + searchVendorId.toUpperCase() + "%' ";
        }

        const result = await axios.post('/api/reports/purchase/gateEntry/filterVendorList', { page, where })
        if (result.data) {
            setVendorList(result.data.vendorList);
    ...
    useEffect(() => {
        if (showVendorlist) {
            if (searchVendorName || searchVendorCode || searchVendorId) {
                filterVendorData();
            } else {
                getVendorList();
            }
    ...
    useEffect(() => {
        filterVendorData();
    }, [searchVendorName, searchVendorCode, searchVendorId,])
    ...
    const getReports = async () => {
        ...
        setLoader(true);
        try {
            const res = await axios.post('/api/reports/purchase/gateEntry/getReports', 
            { orgId, oprId, venApmId, tooDate, fromDate })

            if (res.data.reportsData.length > 0) {
                if (repType === 'H') {
                    await GateReportHtml(res.data.reportsData, "Gate Entry Report");
                } else if (repType === 'P') {
                   await GateReportPdf(res.data.reportsData, "Gate Entry Report");

                } else if (repType === 'E') {
                   await DownloadExcel(res.data.reportsData, "Gate Entry Report");
        ...
        <div >
            ...
            <div className='parentDivPuForm' >
                ...
                <div className='formContDiv' style={{ width: '70%', marginLeft: helpScreen ? '2%' : 'auto', minHeight: '92vh', maxHeight: 'auto' }}>
                    <FormHeading adrmRightId='60307' headingText='Gate Entry Report' />
                    ...
                    <div style={{ display: 'flex', height: '30vh', width: '80%', position: 'relative', marginTop: '3%', marginRight: 'auto', marginLeft: 'auto' }}>
                        ...
                        <div style={{ display: 'flex' }}>
                            <div style={{ height: '4vh', width: '35%', textAlign: 'center', position: 'absolute', left: '15%', top: '52%' }}>
                                <InputTagWithLabel text='Vendor Name' fontSize='0.9rem' searchWidth='63%' funCall={() => { getVendorList(); setShowVendorlist(true) }}
                                    onChange={(e) => setVendorCode(e.target.value)} placeholder="Select Vendor" value={vendCode} display='true'/>
                            </div>
                            <div style={{ height: '4vh', width: '27%', textAlign: 'center', position: 'absolute', left: '55%', top: '52%' }}>
                                <InputTagWithLabel text='' fontSize='0.9rem' display='none' searchWidth='100%'
                                    onChange={(e) => setVendor(e.target.value)} placeholder="Vendor Name" value={vendor} />
                            </div>
                        </div>

                    </div>
                    <div style={{ width: '20%', display: 'flex', justifyContent: 'space-between', marginLeft: 'auto', 
