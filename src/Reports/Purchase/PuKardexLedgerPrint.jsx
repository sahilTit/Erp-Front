...
const PuKardexLedgerPrint = () => {
    ...
    // DuplicateWindowCheck('PuKardexLedgerPrint');
    ...
    const searchDeptInfo = async () => {
        if (searchDepartName) {
            ...
            // console.log("searchDepartId function called");
        }
        else {
            // console.log("nothing");
    ...
    //     // console.log("Required data for post API:-", FinYear, oprId, apmId, transTypeVal, page);
    //     setShowMrsList(true);
    //     try {
    ...
    const searchItemGroupDesc = async () => {
        if (searchItemCode) {
            try {
                ...
                setItemCodeList(result.data.mrsDataList);
                // console.log(result.data, result.data.mrsDataList.length);
    ...
    const printReports = async () => {
        const mon = format(new Date(fromDate), 'MM');
        const year = format(new Date(fromDate), 'yyyy');
        ...
        const formName = 'Kardex Ledger Report';
        ...
        const params = {
            ...
            dt_fromDate: newDate,
            dt_toDate: toDate,
            int_finYear: parseInt(l_lyear),    //fromMRSNo,
            int_month: parseInt(mon),
            s_deptCd: apmId,
            s_itemCd: itemCode,
            s_itemGroupCd: itemGrup,
            s_ColCd: itemColorCode
        }

        if (selectedOption === '1') {
            setLoader(true);
            const res = await onPrintRepJAS(outTypeVal, formName, '/reports/Purchase/PuKardexLedgerPrintRep_Value', params);
            setLoader(false);
        }
        else if (selectedOption === '2') {
            setLoader(true);
            const res = await onPrintRepJAS(outTypeVal, formName, '/reports/Purchase/PuKardexLedgerPrintRep', params);
            if (res)
            setLoader(false);
        }
    }
        ...
        <>
            ...
            <div className='parentDivEpForm' style={{ position: 'relative' }}>
                ...
                <div className='formContDiv' style={{ width: '80%', marginLeft: helpScreen ? '2%' : 'auto' }}>
                    <FormHeading adrmRightId='605' headingText='Purchase Kardex Ledger' />
                    <div style={{ width: '80%', paddingLeft: '10%', margin: '5% auto 0% auto', height: '50vh', zIndex: '1' }}>
                        ...
                        <div style={{ display: 'flex', width: '50vw', height: '5vh', position: 'relative', marginBottom: '2%' }}>
                            <div className='userIdDiv' style={{ marginLeft: '0%', marginTop: '0%', width: '47%', marginBottom: '2%' }}>
                                <InputTagWithLabel width='80%' text='Group Code' searchWidth='68%' funCall={() => { getItemGrupList(); setShowItemGrup(true); }} value={itemGrup}
                                    onChange={(e) => setItemGrup(e.target.value)} placeholder="Select Group" fontSize='1rem' display='true'/>
                            </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', width: '30vw', height: '5vh', marginTop: '3%', marginLeft: '-1.7%' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', width: '70%' }}>
                                <div className={`radio ${selectedOption === '1' ? 'selected' : ''}`} onClick={() => handleRadioClick('1')}>
                                    <label>
                                        <input type="radio" value="1" name="options" style={{ cursor: 'pointer' }} checked={selectedOption === '1'} />
                                        <span className="ps-1" style={{ fontSize: '14px' }}> With Value </span>
                                    </label>
                                </div>
                                <div className={`radio ${selectedOption === '2' ? 'selected' : ''}`} onClick={() => handleRadioClick('2')}>
                                    <label>
                                        <input type="radio" value="2" name="options" style={{ cursor: 'pointer' }} checked={selectedOption === '2'} />
                ...
                <Draggable>
                    <div className="popup-overlay" style={{ width: '30%', position: 'absolute', top: '20vh', left: '35vw', right: 'auto', backgroundColor: '#fff', cursor: 'pointer', zIndex: '5' }}>
                        <div className="popup" style={{ width: 'auto', height: 'auto', padding: '2% 1% 1% 1%', textAlign: 'center', position: 'relative', boxShadow: '1px 1px 2px black', backgroundColor: '#fff' }}>
                            <img src={RemoveImg} alt="" sr et="" style={{ width: '35px', height: '30px', position: 'absolute', right: '0', top: '0' }} onClick={() => { setShowDepatList(false); setSearchDepartName(''); setSearchDepartId(''); setPage(1) }} />
                                    ...
                                    <tbody>
                                        {
                                            itemCodeList.map((itemCode, index) => {
                                                return (<>
                                                    <tr key={index} onClick={() => { setItemCode(itemCode.PUIM_CD); setItemCodeName(itemCode.PUIM_DESC); setShowItemCode(false);setsearchItemCode(''); setsearchItemGroup(''); setsearchItemDesc(''); setPage(1) }} style={{ textAlign: 'left' }}>
...
export default PuKardexLedgerPrint;