...
const PurchaseVouPostInterPlt = () => {
    ...
    const [rightId, setRightId] = useState('6268');
    ...
    DuplicateWindowCheck('PurchaseVouPostInterPlt');
    ...
    const saveFunCall = async () => {
        ...
        if(!acId){
            alert('Select AC Code !');
            return;
        }
      
        if(!trans){
            alert('Select Trans. Code !');
            return;
        }
        if(!tax){
        ...
        try {
            if (grinListFull) {
                const result = await axios.post('/api/forms/Finance/purchaseVouPstInt/generateVoucher',
                    { orgId, oprUnitId,grinListFull,partyId,acId,trans,tax,tds,userId});
    ...
    const getPartyList = async () => {
        ...
        if (searchPartyId !== undefined && searchPartyId !== null && searchPartyId !== '') {
            where = where + ` and APM_CD LIKE ` + "'%" + searchPartyId.toUpperCase() + "%' ";
        }
        if (searchPartyName !== undefined && searchPartyName !== null && searchPartyName !== '') {
            where = where + ` and APM_NAME LIKE ` + "'%" + searchPartyName.toUpperCase() + "%' ";
        }

        const result = await axios.post('/api/forms/Finance/purchaseVouPstInt/getFaGrinVouPartyList', { orgId, where, page })
        if (result.data) {
    ...
    const getAcIdList = async () => {
        ...
        if (searchAcId !== undefined && searchAcId !== null && searchAcId !== '') {
            where = where + ` and acm_short_desc LIKE ` + "'%" + searchAcId.toUpperCase() + "%' ";
        }
        if (searchAcIdName !== undefined && searchAcIdName !== null && searchAcIdName !== '') {
            where = where + ` and acm_desc LIKE ` + "'%" + searchAcIdName.toUpperCase() + "%' ";
        }

        const result = await axios.post('/api/forms/Finance/purchaseVouPstInt/getFaGrinVouAcidCdList', { orgId, where, pageAcId })
        if (result.data) {
            // console.log(result.data.rows);            
    ...
    const getTdsList = async () => {
        ...
        const result = await axios.post('/api/forms/Finance/purchaseVouPstInt/getFaGrinVouTdsCdList', { orgId, oprUnitId })
        if (result.data) {
            console.log(result.data);
    ...
    const retriveGrinList = async () => {
        ...
        try {
            // setPartyId('7303'); 
            const result = await axios.post('/api/forms/Finance/purchaseVouPstInt/getFaGrinVouGrinList', { orgId, oprUnitId, partyId, fromDate, toDate })
            if (result.data) {
    ...
    const handleInvNo = (val,obj, index) => {
        // console.log("function data is", obj,index);
        const updatedData = [...grinListFull];
        obj.PUGH_VEN_INV_NO = val;
        // console.log("function data is",  obj,index);
        setGrinListFull(updatedData);
    }
    const handleInvDt = (val,obj, index) => {
        // console.log("function data is", obj,index);
        const updatedData = [...grinListFull];
        obj.PUGH_VEN_INV_DT = val;
        // console.log("function data is",  obj,index);
        setGrinListFull(updatedData);
    }
    const handleInvAMT = (val,obj, index) => {
        // console.log("function data is", val,index);
        const updatedData = [...grinListFull];
        obj.PUGH_GRSVAL = val;
        // console.log("function data is",  obj,index);
        setGrinListFull(updatedData);
    }
    const handleInvNarr = (val,obj, index) => {
        // console.log("function data is", val,index);
        const updatedData = [...grinListFull];
        obj.NARR = val;
        // console.log("function data is",  obj,index);
        ...
        <>
            ...
            <div className='parentDivEpForm'>
                ...
                <div className='formContDiv' style={{ marginLeft: helpScreen ? '2%' : 'auto', width: '70%' }}>
                    <FormHeading adrmRightId='6260' headingText='Purchase Voucher Posting' />
                        ...
                        <div style={{ position: 'relative', left: '5%' }}>
                            ...
                            <div style={{ display: 'flex', width: '50vw', height: '5vh', position: 'relative', marginBottom: '2%' }} >
                                <div className='userIdDiv' style={{ marginLeft: '0%', marginTop: '0%', width: '45%', marginBottom: '2%' }}>
                                    <InputTagWithLabel display = {disable} width='80%' readOnly='true' text='Party' searchWidth='66%' funCall={() => { getPartyList(); setShowPartyList(true); }} value={partyCd} onChange={(e) => setPartyCd(e.target.value)} placeholder="Select Party Name" fontSize='1rem' />
                                </div>
                                <div className='userIdDiv' style={{ height: '4vh', width: '50%', position: 'absolute', top: '0%', right: '5%', margin: '0%', padding: '0%' }}>
                            ...
                            <div style={{ display: 'flex', width: '61%', left: '5.5%' }}>
                                <div style={{ height: '4vh', width: '90%', textAlign: 'center' }}>
                                    <InputTagWithLabel text='From Date' fontSize='0.9rem' display='none' searchWidth='60%' placeholder="Select From Date" value={fromDate === epochDate ? '' : fromDate instanceof Date ? fromDate.toLocaleDateString() : ''} />
                                </div>
                                <img src={cal} alt='Calender' style={{ width: '30px', height: '30px', cursor: 'pointer' }} onClick={() => setFrDateCal(!frDateCal)} />
                                &nbsp;&nbsp;
                                <div style={{ height: '4vh', width: '90%', textAlign: 'center' }}>
                                    <InputTagWithLabel text='To Date' fontSize='0.9rem' display='none' searchWidth='60%' placeholder="Select To Date" value={toDate === epochDate ? '' : toDate instanceof Date ? toDate.toLocaleDateString() : ''} />
                                </div>
                                <img src={cal} alt='Calender' style={{ width: '30px', height: '30px', cursor: 'pointer' }} onClick={() => setToDateCal(!toDateCal)} />
                            </div>
                            {
                                frDateCal ?
                                    <Draggable>
                                        <div style={{ marginLeft: '5%', marginTop: '3%', marginBottom: '5%', zIndex: '10' }}>
                                            <Calendar onChange={(fromDate) => { setFromDate(fromDate); setFrDateCal(false); }} value={fromDate} width='12%' height='20%' />
                                        </div></Draggable> : <></>
                            ...
                            }
                            {
                                toDateCal ?
                                    <Draggable>
                                        <div style={{ marginRight: '5%', marginTop: '3%', marginBottom: '5%', zIndex: '1' }}>
                                            <Calendar onChange={(toDate) => { setToDate(toDate); setToDateCal(false); }} value={toDate} width='12%' height='20%' />
                                        </div></Draggable> : <></>

                            }
                            <div style={{ display: 'flex', width: '50vw', height: '5vh', position: 'relative', marginBottom: '2%' }} >
                                <div className='userIdDiv' style={{ marginLeft: '0%', marginTop: '0%', width: '45%', marginBottom: '2%' }}>
                                    <InputTagWithLabel display = {disable} readOnly='true' width='80%' text='AC Code' searchWidth='66%' funCall={() => { getAcIdList(); setShowAcIdList(true); }} value={acCd} onChange={(e) => setAcId(e.target.value)} placeholder="Select AcId Name" fontSize='1rem' />
                                </div>
                                <div className='userIdDiv' style={{ height: '4vh', width: '50%', position: 'absolute', top: '0%', right: '5%', margin: '0%', padding: '0%' }}>
                            ...
                            <div style={{ height: '4vh', width: '19.9vw', textAlign: 'left', marginBottom: '1.5%', display: 'flex' }}>
                                <label style={{ textAlign: 'right', marginBottom: '1.5%', fontSize: '14px' }}>Trans. Code</label>
                            ...
                            <div style={{ height: '4vh', width: '19.9vw', textAlign: 'left', marginBottom: '1.5%', display: 'flex' }}>
                                <label style={{ textAlign: 'right', marginBottom: '1.5%', fontSize: '14px' }}>Tax</label>
                                <select className='dropdown-button'
                            ...
                            <div style={{ height: '4vh', width: '19.9vw', textAlign: 'left', marginBottom: '1.5%', display: 'flex' }}>
                                <label style={{ textAlign: 'right', marginBottom: '1.5%', fontSize: '14px' }}>Type of payment</label>
                                <select className='dropdown-button'
                            ...
                            <button className='btn btn-danger btn-sm p-0 ps-3 pe-3 pt-1 pb-1 ml-3'
                                style={{ alignItems: 'center', verticalAlign: 'middle', fontSize: '10px' }} onClick={() => retriveGrinList()}>Retrive
                            </button>
                            ...
                            <div style={{ minHeight: '48vh', maxHeight: 'auto', marginBottom: '5%', marginTop: '1%' }}>
                                <table className="table table-bordered" style={{ width: '100%', marginLeft: 'auto', marginRight: 'auto', backgroundColor: '#fff' }} >
                                    <thead>
                                        <tr style={{ textAlign: 'left', backgroundColor: '#D9F3FF' }}>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '6%' }} >Action</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '6%' }} >Grin No</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '10%' }}>Grin Dt</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '10%' }}>Invoice</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '10%' }}>Invoice Dt</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '7%' }}>Invoice AMT</th>
                                            <th className="p-0 ps-4 pb-1" style={{ width: '25%' }}>Narrtion</th>
                                    ...
                                    <tbody>
                                        {
                                            grinList.length ? grinList.map((item, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '100%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.PUGH_NO} placeholder={'Grin No'} />
                                                        </td>
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '100%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.PUGH_FINAL_GRN_DT} placeholder={'Grin Dt'} />
                                                        </td>
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '100%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.PUGH_VEN_INV_NO} onChange={(e) => handleInvNo(e.target.value,item, index)} placeholder={'Invoice'} />
                                                        </td>
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '100%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.PUGH_VEN_INV_DT} onChange={(e) => handleInvDt(e.target.value,item, index)} placeholder={'Invoice Dt'} />
                                                        </td>
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '95%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.PUGH_GRSVAL} onChange={(e) => handleInvAMT(e.target.value,item, index)} placeholder={'Invoice AMT'} />
                                                        </td>
                                                        <td className='p-0 pt-1 text-left ps-1' style={{ fontSize: '12px' }}>
                                                            <input className="inputTagIn" type={'text'} style={{ width: '95%', height: '4vh', backgroundColor: '#EFFAFF', fontSize: '12px' }}
                                                                value={item.NARR} onChange={(e) => handleInvNarr(e.target.value,item, index)} placeholder={'Narrtion'} /> 
                                                        </td>
                                ...
                                <Pagination totalPage={totalGrn} page={pageGrn} limit={limit} siblings={1} onPageChange={handlePageGrinChange} />
                            </div>
...
export default PurchaseVouPostInterPlt;