...
const QaWaterTest = () => {
    ...
    DuplicateWindowCheck('QaWaterTest');
    ...
    const getModifyList = async () => {
        const result = await axios.post('/api/forms/QA/dashBoardAllRoute/getModifyWaterTstList')
        if (result.data) {
            // console.log(result.data.resData);
    ...
    const safetyMonSave = async () => {
        // console.log('method call');
        try {
            const result = await axios.post('/api/forms/QA/dashBoardAllRoute/saveWaterTest', { safetyMon, standard, repVal, entryDate, userId, mode })
            if (result) {
    ...
    const setModifyValue = (item) => {
        console.log(item);
        setSafetyMon(item.TEST_COND);
        setStandard(item.STANDARD);
        setRepVal(item.REP_VAL);
        ...
        <>
            ...
            <div className='parentDivEpForm'>
                ...
                <div className='formContDiv' style={{ marginLeft: helpScreen ? '2%' : 'auto', width: '70%' }}>
                    <FormHeading adrmRightId='6260' headingText='QA Water Test' />
                    <div style={{ width: '90%', minHeight: '20vh', maxHeight: 'auto', marginLeft: 'auto', marginRight: 'auto', paddingLeft: '0%', paddingBottom: '3%' }}>
                        <div style={{ position: 'relative', left: '5%' }}>
                            ...
                            <div style={{ height: '4vh', width: '19.9vw', textAlign: 'left', marginBottom: '1.5%', display: 'flex' }}>
                                <label style={{ textAlign: 'right', marginBottom: '1.5%', fontSize: '14px' }}>Test Cond</label>
                            ...
                            <div style={{ display: 'flex', width: '31%', left: '5.5%', marginBottom: '2%' }}>
                                <div style={{ height: '4vh', width: '90%', textAlign: 'center' }}>
                                    <InputTagWithLabel text='Standard' display='none' fontSize='0.9rem' searchWidth='60%' placeholder="fill Standard" value={standard} onChange={(e) => setStandard(e.target.value)} />
                                </div>
                            </div>
                            <div style={{ display: 'flex', width: '31%', left: '5.5%', marginBottom: '2%' }}>
                                <div style={{ height: '4vh', width: '90%', textAlign: 'center' }}>
                                    <InputTagWithLabel text='Report Value' display='none' fontSize='0.9rem' searchWidth='60%' placeholder="Fill Report Value" value={repVal} onChange={(e) => setRepVal(e.target.value)} />
                                </div>
                            </div>
                                            ...
                                            <tbody>
                                                {
                                                    modData.map((item, index) => {
                                                        return (<tr key={index} onClick={() => { setModifyValue(item); }}>
                                                            ...
                                                            <td className="p-0 ps-3">{item.STANDARD}</td>
                                                            <td className="p-0 ps-3">{item.REP_VAL}</td>
